                ...
                <div className="cus">
                    ...
                    <div className="cus_info">
                        <div>
                            ...
                            <h3>{data2 && data2.PolicyNo ? data2.PolicyNo : 'No Policy number yet'}</h3> 
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <h3>{data.phoneBrand === undefined || data.phoneModel === undefined ? 'fetching...' : (data.phoneBrand + ' ' + data.phoneModel)}</h3> 
                        </div>
                        <div>
                            <span>Phone Value</span>
                            <h3>{data.phoneValue === '' || data.phoneValue === undefined ? '' : data.phoneValue}</h3> 
                        </div>
                        <div>
                            <span>Year of Purchase</span>
                            <h3>{data.yearOfPurchase === '' || data.yearOfPurchase === undefined ? '' : data.yearOfPurchase}</h3> 
                        </div>
                        <div>
                            <span>Phone Condition</span>
                            <h3>{data.phoneCondition === '' || data.phoneCondition === undefined ? '' : data.phoneCondition}</h3> 
                        </div>
                    </div>
                </div>
                <div className="claim">
                ...
                <div className="pay">
                    ...
                    <div className="pay_info d-flex flex-column justify-content-between h-75">
                        <div>
                            ...
                            <h3>{data.phoneValue === '' || data.phoneValue === undefined ? '' : Number(data.phoneValue) * 0.03}</h3> 
                        </div>
                        <div>
                            <span>Status</span>
                            <h3>{data.payment === undefined ? 'Not Paid' : data.payment}</h3> 
                        </div>
                        <form className='w-100 mb-4' onSubmit={PolicyNoSubmit}>
                        <div className="mb-2">
                            <input type="text" className="form-control" placeholder="Add Policy Number" required onChange={(e) => setPolicyNo(e.target.value)} value={policyNo}  />
                        </div>
                            <button className='btn gen-btn'>Add Policy Number</button>
                        </form>
                        <form className='w-100' onSubmit={CertSubmit}>
                        <div className="input-div d-flex flex-column">
                            <input type="file" required accept='image/*' id='damImg' onChange={(e) => setFile(e.target.files[0])} name='damImg' />
